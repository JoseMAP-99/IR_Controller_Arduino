...
/******PIANO****/
boolean modoPiano = false;
volatile byte tecladoBoton = 0x00;
volatile int contPiano = 0;
/***************/
...
void loop() {
  ...
  if(teclaPulsado == 0x06 && cont == 3 && !modoPiano || teclaPulsado == 0x06 && contJuego == 3 && !modoPiano){
  ...
  }else if(teclaPulsado == 0x0C && !modoJuego && cont == 1 || teclaPulsado == 0x0C && !modoJuego && contJuego == 1 || cambio && !modoJuego){ //Entra a juego, * y 7.    
    if(millis() > timeTecla + 200){
      timeTecla = millis();
      //"Limpia" la pantalla LCD.
      Serial3.write(0xFE);
  ...
  }else if(teclaPulsado == 0x06 && modoJuego && cont == 1 || teclaPulsado == 0x06 && modoJuego && contJuego == 1 || PINC >> 3 == 0x16 && modoPiano || !cambio && modoJuego){ //Sale de juego, * y 1.
    if(millis() > timeTecla + 200){
      timeTecla = millis();
      //Limpia la pantalla.
      Serial3.write(0xFE);
...
/***********Rutina del piano********/
void ISR_Piano(){  
  PORTL = segmentos[contPiano]; //Se habilita el dígito correspondiente.
  ...
  if(contPiano == 4) contPiano = 0; //Si se ha llegado al último dígito del 7-segmentos, se reinicia a 0. 
}
/***********************************/

/**********Rutina teclado del piano***********/
void reboteTecladoPiano(){
  ...
  switch(teclaPulsado){
    case 0x07: //Si se ha pulsado la primera fila {1, 2, 3}.
              if (millis() > timeTecla + 200) {
                ...
                timeTecla = millis();
                if(contPiano == 0){ //Se ha activado las unidades.
                    tecladoBoton = num[1]; //Valor 1. 
                    tone(37, 277, 200);                       
                }else if(contPiano == 1){ //Se ha activado las decenas.
                    tecladoBoton = num[2]; //Valor 2. 
                    tone(37, 294, 200);     
                }else if(contPiano == 2){ //Se ha activado las centenas.
                    tecladoBoton = num[3]; //Valor 3.
                    tone(37, 311, 200);   
                }
              }
              break;
    case 0x0B: //Si se ha pulsado la segunda fila {4, 5, 6}.
              if (millis() > timeTecla + 200) {
                timeTecla = millis();
                if(contPiano == 0){ //Se ha activado las unidades.
                    tecladoBoton = num[4]; //Valor 4.   
                    tone(37, 330, 200);     
                }else if(contPiano == 1){ //Se ha activado las decenas.
                    tecladoBoton = num[5]; //Valor 5.   
                    tone(37, 349, 200);     
                }else if(contPiano == 2){ //Se ha activado las centenas.
                    tecladoBoton = num[6]; //Valor 6.
                    tone(37, 370, 200);   
                }
              }
              break;
    case 0x0D: //Si se ha pulsado la tercera fila {7, 8, 9}.
              if (millis() > timeTecla + 200) {
                timeTecla = millis();
                if(contPiano == 0){ //Se ha activado las unidades.
                    tecladoBoton = num[7]; //Valor 7.  
                    tone(37, 392, 200);        
                }else if(contPiano == 1){ //Se ha activado las decenas.
                    tecladoBoton = num[8]; //Valor 8.   
                    tone(37, 415, 200);     
                }else if(contPiano == 2){ //Se ha activado las centenas.
                    tecladoBoton = num[9]; //Valor 9.
                    tone(37, 440, 200);   
                }
              }
              break;
    case 0x0E: //Si se ha pulsado la cuarta fila {0}.
              if (millis() > timeTecla + 200) {
                if(contPiano == 1){ //Se ha activado las decenas.
                    tecladoBoton = num[0]; //Valor 0.    
                    tone(37, 262, 200);    
                }
              }
              break;           
